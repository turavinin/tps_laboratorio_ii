...
namespace TP3
{
    public partial class FormOperacionesPersonas : Form
    {
        ...
        public FormOperacionesPersonas(EncuestaManager encuestaManager, string tituloVentana, int idPersona = 0)
        {
            ...
            if (idPersona > 0)
            {
                _persona = _encuestaManager.EncontrarRegistroLista<Persona>(idPersona);
            }
        }
        ...
        private void btnAceptarPersona_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    this.EditarPersona();
                    _encuestaManager.ActualizarEntidad(_persona);
                }
                else
                {
                    _encuestaManager.GuardarEntidad(this.CrearPersona());
                }
            }

            _encuestaManager.CargarListaEntidad<Persona>();
            this.Close();
        }
        ...
        private Persona CrearPersona()
        {
            Persona nuevaPersona = new Persona()
            {
                ...
                PuestoId = _encuestaManager.GetIdByNombreLista<Puesto>(this.cmbPuestoPersona.SelectedItem.ToString()),
                ExperienciaId = _encuestaManager.GetIdByNombreLista<Experiencia>(this.cmbExpPersona.SelectedItem.ToString()),
        ...
        private void EditarPersona()
        {
            ...
            _persona.PuestoId = _encuestaManager.GetIdByNombreLista<Puesto>(this.cmbPuestoPersona.SelectedItem.ToString());
            _persona.ExperienciaId = _encuestaManager.GetIdByNombreLista<Experiencia>(this.cmbExpPersona.SelectedItem.ToString());
